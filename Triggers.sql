...
-- Description:	<Cambia el curso de un alumno, actualizando tambi�n sus notas. Se dan tres situaciones:
--				1-El nuevo curso sea v�lido y el alumno ya haya pertenecido a un curso, por lo que se borran sus notas anteriores y se cargan las notas correspondientes al nuevo curso
--				2-El nuevo curso sea v�lido y el alumno no haya pertenecido a un curso, por lo que se cargan las notas correspondientes al nuevo curso
	...
	IF @anioNuevo IS NOT NULL AND @divisionNueva IS NOT NULL
		BEGIN
			...
			BEGIN
				...
				ROLLBACK TRAN
			END
			/*Si el alumno no pertenec�a a un curso, solo se deben agregar las notas del curso al que se agreg�*/
		...
		END
	ELSE
	/*Si llega hasta aqu� quiere decir que anio y/o division son NULL, por lo que si el alumno ya pertenec�a a un curso debemos borrar sus notas*/
	BEGIN
